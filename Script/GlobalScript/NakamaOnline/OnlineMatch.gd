...
func _set_nakama_socket(_nakama_socket: NakamaSocket) -> void:
	print("Match_mode in _set_nakama_socket ", OnlineMatch.match_mode)
...
func start_matchmaking(_nakama_socket: NakamaSocket, data: Dictionary = {}) -> void:
	...
	print("start_matchmaking_state OnlineMatch:", match_state)
	print("start_matchmaking_mode OnlineMatch before result:", match_mode)
	...
	print("start_matchmaking_mode OnlineMatch after result:", match_mode)
	match_mode = MatchMode.MATCHMAKER
	print("start_matchmaking_mode OnlineMatch after result2:", match_mode)
	if result.is_exception():
		print("start matchmaking_exception: ", result.get_exception().message)
		leave()
		emit_signal("error", "Unable to join match making pool")
	else:
		print("start matchmaking_mode OnlineMatch no exception:", match_mode)
...
func leave(close_socket: bool = false) -> void:
	...
	print("Match_mode in leave begin ", match_mode)
	# Nakama disconnect.
	if nakama_multiplayer_bridge:
		nakama_multiplayer_bridge.leave()
	if nakama_socket:
	...
	print("Match_mode in leave end ", match_mode)
...
func _on_match_joined() -> void:
	...
	print("Match_joined in OnlineMatch: ", match_mode)
